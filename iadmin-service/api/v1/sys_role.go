...
// @accept application/json
// @Produce application/json
// @Param data body request.SysRoleApiConfig true "role_id,id列表"
// @Success 200 {string} string "{"success":true,"data":{},"msg":"配置角色接口成功"}"
// @Router /api/sysRole/updateSysRoleApiConfig [PUT]
func UpdateSysRoleApiConfig(c *gin.Context)  {
	var s request.SysRoleApiConfig
	_ = c.ShouldBindJSON(&s)
	if err := service.UpdateSysRoleApiConfig(s); err != nil {
		global.GvaLog.Error("配置角色接口失败!", zap.Any("err", err))
		response.FailWithMessage("配置角色接口失败！", c)
	} else {
		response.OkWithMessage("配置角色接口成功", c)
	}
}